...
const sendKeysToCollection = applyToCollection('sendKeys');

const createLocatorConfig = (nemo, drivex, options, locator, page) => {
  const normalizedLocator = normalize(nemo, locator);
  const parentLocator = (
    (!options.resetScope && page.scope) &&
    drivex.find(normalize(nemo, page.scope))
  );

  const useLocator = (
    options.scope &&
    drivex.find(normalize(nemo, options.scope), parentLocator)
  );
...
function asCollection(multiple, at) {
  return (collection) => {
    let index = at;
  ...
  return function getText(locator, options = {}) {
    return (page) => {
      ...
      return drivex
        ...
        .then(asCollection(options.multiple, options.at))
  ...
  return function fill(locator) {
    return (page, value, options) => {
      ...
      return drivex
        .finds(config.locator, config.useLocator)
        .then(asCollection(options.multiple, options.at))
        .then(sendKeysToCollection(value))
        .then(getItemAtCollectionIndex(options.multiple, options.at));
    }
  };
}
...
function collection(nemo, drivex) {
  return function createCollection(options) {
    return (page, index) => {
      const config = createLocatorConfig(nemo, drivex, options, options.itemScope, page);
