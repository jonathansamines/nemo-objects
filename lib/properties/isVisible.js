...
function isVisibleProperty(nemo, control, descriptor) {
  /**
  ...
  return function isVisible(locator, options = {}) {
    return async (page) => {
      ...
      const areElementsVisible = await Promise.all(elements.map(el => el.isDisplayed()));
...
module.exports = isVisibleProperty;
